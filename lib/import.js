 ...
 */
export async function upsertAll(payload, { rawSizeBytes } = {}) {
  ...
  try {
    ...
    for (const item of itemRecords) {
      ...
      // Persist photo to configured storage (db or fs); resolves to storable value
      const photoUrl = photo.dataUrl
        ? await savePhoto(item.id, photo.dataUrl)
        : null;

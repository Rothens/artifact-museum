import { getDbSync, persist } from "../../db/client.js";

function queryOne(sql, params = []) {
  const db = getDbSync();
  const stmt = db.prepare(sql);
  stmt.bind(params);
  const row = stmt.step() ? stmt.getAsObject() : null;
  stmt.free();
  return row;
}

function queryAll(sql, params = []) {
  const db = getDbSync();
  const stmt = db.prepare(sql);
  stmt.bind(params);
  const rows = [];
  while (stmt.step()) rows.push(stmt.getAsObject());
  stmt.free();
  return rows;
}

export function getItemById(id) {
  return queryOne("SELECT * FROM item_records WHERE id = ?", [id]);
}

export function getItemsByCodeId(codeDefinitionId) {
  return queryAll(
    "SELECT * FROM item_records WHERE code_definition_id = ? ORDER BY collected_at DESC",
    [codeDefinitionId]
  );
}

export function getPublicItemByCodeId(codeDefinitionId) {
  return queryOne(
    "SELECT * FROM item_records WHERE code_definition_id = ? AND is_public = 1 ORDER BY collected_at DESC LIMIT 1",
    [codeDefinitionId]
  );
}

export function getAllPublicItems() {
  return queryAll(`
    SELECT ir.*, cd.name AS code_name, cd.category, cd.mode
    FROM item_records ir
    JOIN code_definitions cd ON cd.id = ir.code_definition_id
    WHERE ir.is_public = 1
    ORDER BY ir.collected_at DESC
  `);
}

export function getAllItemsWithStats() {
  return queryAll(`
    SELECT ir.*, cd.name AS code_name, cd.category, cd.mode,
           COUNT(pv.id) AS view_count
    FROM item_records ir
    JOIN code_definitions cd ON cd.id = ir.code_definition_id
    LEFT JOIN page_views pv ON pv.item_id = ir.id
    GROUP BY ir.id
    ORDER BY cd.name ASC, ir.collected_at DESC
  `);
}

export function updateItemVisibility(id, flags) {
  const db = getDbSync();
  const {
    is_public,
    show_photo,
    show_notes,
    show_price,
    show_source_shop,
    show_recipient,
    show_consumed,
    show_gifted,
    show_location,
    admin_notes,
    label,
    notes,
  } = flags;

  db.run(
    `UPDATE item_records SET
      is_public = ?,
      show_photo = ?,
      show_notes = ?,
      show_price = ?,
      show_source_shop = ?,
      show_recipient = ?,
      show_consumed = ?,
      show_gifted = ?,
      show_location = ?,
      admin_notes = ?,
      label = ?,
      notes = ?
    WHERE id = ?`,
    [
      is_public ? 1 : 0,
      show_photo ? 1 : 0,
      show_notes ? 1 : 0,
      show_price ? 1 : 0,
      show_source_shop ? 1 : 0,
      show_recipient ? 1 : 0,
      show_consumed ? 1 : 0,
      show_gifted ? 1 : 0,
      show_location ? 1 : 0,
      admin_notes ?? "",
      label ?? "",
      notes ?? "",
      id,
    ]
  );
  persist();
}

/**
 * Update the raw data fields of an item (photo, price, shop, etc.).
 * Pass null for photo_data_url to clear the photo.
 * Pass undefined to leave it unchanged.
 */
export function updateItemData(id, data) {
  const db = getDbSync();
  const sets = [];
  const params = [];

  if ("photo_data_url" in data) {
    sets.push("photo_data_url = ?", "photo_width = ?", "photo_height = ?", "photo_size = ?");
    params.push(
      data.photo_data_url ?? null,
      data.photo_width ?? null,
      data.photo_height ?? null,
      data.photo_size ?? null
    );
  }
  if ("meta_price" in data) { sets.push("meta_price = ?"); params.push(data.meta_price ?? null); }
  if ("meta_currency" in data) { sets.push("meta_currency = ?"); params.push(data.meta_currency ?? null); }
  if ("meta_source_shop" in data) { sets.push("meta_source_shop = ?"); params.push(data.meta_source_shop ?? null); }
  if ("meta_recipient" in data) { sets.push("meta_recipient = ?"); params.push(data.meta_recipient ?? null); }
  if ("meta_consumed" in data) { sets.push("meta_consumed = ?"); params.push(data.meta_consumed ? 1 : 0); }
  if ("meta_gifted" in data) { sets.push("meta_gifted = ?"); params.push(data.meta_gifted ? 1 : 0); }

  if (sets.length === 0) return;
  params.push(id);
  db.run(`UPDATE item_records SET ${sets.join(", ")} WHERE id = ?`, params);
  persist();
}
